...
function readSettings(callback) {
    ...
    fs.exists(deploySettings, function(exists){
        if (!exists) {
            console.log('ERROR: .deploy file not found in ' + deploySettings);
            process.exit(1);
        }
...
readSettings(function(err, hosting){
    check(err);
    console.log('deploy to: http://' + hosting.url.hostname + ':' + hosting.url.port);
